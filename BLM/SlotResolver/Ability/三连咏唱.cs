...
public class 三连咏唱 : ISlotResolver
{
    ...
    public int Check()
    {
        ...
        if (BattleData.Instance.火循环剩余gcd<2 && (new 雷1().Check() < 0 || new 雷2().Check() < 0) &&
            Spells.即刻.GetSpell().Cooldown.TotalSeconds > 3 && BLMHelper.火状态 && !QT.Instance.GetQt("保留三连")) return 3;
        if (BLMHelper.火状态 && Spells.三连.GetSpell().Charges * 60 >= 110 && !QT.Instance.GetQt("保留三连")) return 1;
        return -99;
    }
}
