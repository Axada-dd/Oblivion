...
public class 火三:ISlotResolver
{
    public int Check()
    {
        ...
        if (!new Spell(154, SpellTargetType.Target).IsReadyWithCanCast()) return -2;
        ...
        if(BLMHelper.冰状态&&!Core.Me.HasAura(Buffs.火苗)&&BLMHelper.火层数>=3&&BLMHelper.冰针>=3&&Core.Me.CurrentMp>=10000)return 1;
