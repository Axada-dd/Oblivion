...
public class 绝望 : ISlotResolver
{
    ...
    public int Check()
    {
        ...
        if (BLMHelper.火状态 && BLMHelper.火层数 == 3 && BLMHelper.耀星层数 == 6 && Core.Me.CurrentMp <= 2400 && Helper.IsMove && !BattleData.Instance.可瞬发) return 1;
        if (BLMHelper.火层数 == 3 && BattleData.Instance.已使用耀星 && Core.Me.CurrentMp <= 2400) return 3;
        if (BLMHelper.火状态 && BLMHelper.火层数 <= 3 && Core.Me.CurrentMp < 2400 && BLMHelper.耀星层数 < 6 && !Core.Me.HasAura(Buffs.火苗)) return 2;
        return -99;
    }
}
